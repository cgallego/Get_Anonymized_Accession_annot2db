...
def update_table(PatientID, StudyID, accession):
    ...
    try:
        ...
        flog=open('script_log.txt','a')
        flog.write('Adding to biomatrix series table ERROR:'+PatientID+'\t'+StudyID+'\t'+accessnum+'\t'+str(e)+'\n')
        flog.close()
    return
...
def check_pacs(path_rootFolder, remote_aet, remote_port, remote_IP, local_port, PatientID, StudyID, AccessionN):
    ...
    readQueryFile1 = open(program_loc+os.sep+'checkdata'+os.sep+'checkdata_'+AccessionN+'.txt' , 'r')
    ...
    if flagfound == 1:
        ...
        fil.close()
        flog=open('script_log.txt','a')
        flog.write(PatientID+'\t'+StudyID+'\t'+str(AccessionN)+'\tAccession number found in '+remote_aet+'\n')
        flog.close()
        print PatientID+'\t'+StudyID+'\t'+str(AccessionN)+'\tAccession number found in '+remote_aet+'\n'
...
def pull_pacs(path_rootFolder, remote_aet, remote_port, remote_IP, local_port, PatientID, StudyID, AccessionN):
    ...
    if flagfound == 0:
        ...
        fil.close()
        flog=open('script_log.txt','a')
        flog.write(PatientID+'\t'+StudyID+'\t'+str(AccessionN)+'\tAccession number not found in AS0SUNB\n')
        flog.close()
        print "\n===============\n"
        #sys.exit("Error. Accession number not found in AS0SUNB.")
        print "Error. Accession number not found in AS0SUNB."
        return flagfound
